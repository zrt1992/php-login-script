...
class HTTPSession {
    ...
    public function __construct() {
        # Connect to database
        $this->dbhandle = pg_connect("host=db dbname=prophp6 user=ed")  or die
        ("PostgreSQL error:--> " . pg_last_error($this->dbhandle));
        # Set up the handler
        ...
        if ($_COOKIE["PHPSESSID"]) {
            ...
            if (pg_num_rows($result)==0) {
                ...
                $result = pg_query("DELETE FROM \"http_session\" WHERE (ascii_session_id = '". $this->php_session_id . "') OR (now() - created > '$maxlifetime seconds')");
                # Clean up stray session variables
                $result = pg_query("DELETE FROM \"session_variable\" WHERE session_id NOT IN (SELECT id FROM \"http_session\")");
    ...
    public function Impress() {
        if ($this->native_session_id) {
            $result = pg_query("UPDATE \"http_session\" SET last_impression = now() WHERE id = " . $this->native_session_id);
        };
    }
    ...
    public function Login($strUsername, $strPlainPassword) {
        ...
        if (pg_num_rows($result)>0) {
            ...
            $this->user_id = $row["id"];
            $this->logged_in = true;
            $result = pg_query("UPDATE \"http_session\" SET logged_in = true, user_id = " . $this->user_id . " WHERE id = " . $this->native_session_id);
            return(true);
        } else {
            return(false);
        };
    ...
    public function LogOut() {
        if ($this->logged_in == true) {
            $result = pg_query("UPDATE \"http_session\" SET logged_in = false, user_id = 0 WHERE id = " . $this->native_session_id);
    ...
    public function _session_read_method($id) {
        ...
        $result = pg_query("select id, logged_in, user_id from \"http_session\" where ascii_session_id = '$id'");
        if (pg_num_rows($result)>0) {
            $row = pg_fetch_array($result);
            ...
            $result = pg_query("INSERT INTO http_session(ascii_session_id, logged_in,user_id, created, user_agent) VALUES ('$id','f',0,now(),'$strUserAgent')");
            # Now get the true ID
            $result = pg_query("select id from \"http_session\" where
                  ascii_session_id = '$id'");
            $row = pg_fetch_array($result);
            $this->native_session_id = $row["id"];
        };
    ...
    private function _session_destroy_method($id) {
        $result = pg_query("DELETE FROM \"http_session\" WHERE ascii_session_id = '$id'");
        return($result);
    }
